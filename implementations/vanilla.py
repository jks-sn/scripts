...
from utils.sql_templates import generate_create_publication_query, generate_create_schema_query, generate_create_subscription_query, generate_create_table_query, generate_drop_publication_query, generate_drop_schema_query, generate_drop_subscription_query, generate_drop_table_query
...
class Vanilla(DDLInterface):
    ...
    def create_publication(self, publication_name: str, schema_name: str, ddl: bool):
        sql = generate_create_publication_query(publication_name, schema_name, False)
        execute_sql(self.conn_params, sql, self.server_name)
        logger.debug(f"Publication '{publication_name}' created on {self.server_name}")
